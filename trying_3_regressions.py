...
dataset = pd.read_csv('three_month_mod_csv/SOL-USD_mod.csv')
...
regressor2 = RandomForestRegressor(n_estimators=70, criterion="absolute_error",
                                   random_state=0
                                   )
...
for i in range(len(y_pred_list)):
    ...
    cross = cross_val_score(estimator=regressor_list[i], X=x_train, y=y_train, cv=10)
...
# single result
# last_row_dataset = pd.read_csv('three_month_last_row_csv/SOL-USD_last_row.csv')
