...
INTERVAL = 3  # now 3 hours ahead


def close_price_later_and_percent_and_classify(insert_close_price: list) -> tuple:
    close_price_later = insert_close_price[INTERVAL:]
    # print(close_price_later)
    for i in range(INTERVAL):
        close_price_later.append("nan")
    # print(close_price)
    # print(close_price_later)
    increase_percent = []
    buy_signals = []
    for i, x in enumerate(insert_close_price):
        try:
            temp = (close_price_later[i] - insert_close_price[i]) / insert_close_price[i] * 100
            if temp >= 1:
                temp2 = 1  # True, buy signal
            else:
                temp2 = 0  # False, don't buy
        except:
            temp = "nan"
            temp2 = "nan"
        increase_percent.append(temp)
        buy_signals.append(temp2)
    return close_price_later, increase_percent, buy_signals
...
def make_csv_mod_and_last_row(mod_destination, last_row_destination, input_csv_directory, method_of_download):
    ...
    dataset[f"Close Price {INTERVAL} hours later"], dataset["Increase %"], dataset["Buy signal"] = \
        close_price_later_and_percent_and_classify(close_price)

    # remove row0-80 because macd
    # and also row -3 to -2 because nan value for % increase
    # row -1 is kept for single prediction
    dataset.drop(dataset.index[range(80 + 1)], inplace=True, axis=0)
    dataset.drop(dataset.index[range(-INTERVAL, -1)], inplace=True, axis=0)
